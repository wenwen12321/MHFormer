...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.drop_path = nn.Identity() ## 直接給他 Identity()，只會返回原來的值不會做其他事
...
class PoseTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        #################################################################################################
        # 改變維度: [8, 1, 1024]
        x = self.head(x) ############ 改這裡面的 function

        

        # 改變維度: [8, 544, 1, 1] 
        # x = x.view(x.shape[0], 544, 1, 1) ## 改這裡~~~~



        ### 如果不想要 [8, 1, 1024] 維度中的 1 這個 dimension，可以用 squeeze()
        # x = x.squeeze() # 會變 [8, 1024]
        #################################################################################################
        return x